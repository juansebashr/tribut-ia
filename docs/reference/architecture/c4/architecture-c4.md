...
Muestra los bloques ejecutables y tecnologías que conforman la solución en **GCP Cloud Run** y **Redis**.
...
ContainerDb(redis, "Redis Session Store & Pub/Sub", "Redis 7+ (Memorystore / Upstash)", "Almacena estados de sesión (session:{id}) con TTL de 24h y canales Pub/Sub para SSE multihost.")

Rel(user, spa, "Interactúa y visualiza formularios", "HTTPS")
Rel(agent, api, "Llama POST /session/state con Header X-Session-ID", "REST/JSON")
...
Rel(api, redis, "Lee/Escribe estado JSON (TTL 86.400s) y publica eventos", "async redis / PubSub")
```
...
Desglose interno de los módulos del backend FastAPI y la abstracción de sesión.
...
ContainerDb(redis_instance, "Redis Database", "Claves session:{id} y canales Pub/Sub")

Rel(router, session_dep, "Resuelve sesión activa", "Inyección")
Rel(router, session_store, "Lee/Escribe estado y suscribe SSE", "async")
Rel(session_store, redis_instance, "Operaciones GET, SETEX y PUBLISH", "redis-py")
