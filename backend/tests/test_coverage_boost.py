...
from app.services.session_store_base import SessionStoreBase, SessionState
...
from app.core.rules_engine.loader import get_rules_for_year, load_all_rules, get_available_tax_years
...
def test_simulation_compare_pn_with_savings():
    ...
    res = client.post("/api/v1/simulation/compare-pn", json={
        "base_scenario": base,
        "optimized_scenario": opt
    })
...
def test_rules_endpoints_complete():
    ...
    res_c1 = client.post("/api/v1/rules/convert-uvt", json={
        "tax_year": 2026,
        "amount_cop": 52350000.0
    })
    assert res_c1.status_code == 200
    assert res_c1.json()["amount_uvt"] == 1000.0

    # Conversión UVT a COP con custom_uvt
    res_c2 = client.post("/api/v1/rules/convert-uvt", json={
        "tax_year": 2026,
        "custom_uvt": 50000.0,
        "amount_uvt": 100.0
    })
...
def test_session_sync_calc_exception_branches():
    ...
    with patch("app.api.v1.endpoints.session_sync.liquidar_persona_natural", side_effect=ValueError("Test PN Error")):
        res_pn = client.post("/api/v1/session/state?session_id=err_sess", json={
            "persona_natural": {"rentas_trabajo": 1000000}
        })
        assert res_pn.status_code == 200

    with patch("app.api.v1.endpoints.session_sync.liquidar_persona_juridica", side_effect=ValueError("Test PJ Error")):
        res_pj = client.post("/api/v1/session/state?session_id=err_sess_pj", json={
            "persona_juridica": {"ingresos_brutos_operacionales": 1000000}
        })
...
def test_calculate_endpoints_error_branches():
    ...
    with patch("app.api.v1.endpoints.persona_natural.liquidar_persona_natural", side_effect=ValueError("Validation Error")):
        res = client.post("/api/v1/calculate/persona-natural/calculate", json={"tax_year": 2026, "rentas_trabajo": 100})
        assert res.status_code == 400

    with patch("app.api.v1.endpoints.persona_juridica.liquidar_persona_juridica", side_effect=ValueError("Validation Error")):
        res = client.post("/api/v1/calculate/persona-juridica/calculate", json={"tax_year": 2026, "ingresos_brutos_operacionales": 100})
        assert res.status_code == 400
...
def test_beneficios_service_edge_cases():
    ...
    res_75 = calcular_reduccion_sancion(ReduccionSancionRequest(
    ...
    res_none = calcular_reduccion_sancion(ReduccionSancionRequest(
        monto_sancion_base_cop=1000000,
        sin_sanciones_ultimos_2_anos=False,
    ...
    res_no_min = calcular_beneficio_auditoria(BeneficioAuditoriaRequest(
        tax_year=2026,
...
async def test_session_store_base_abstract_methods():
    ...
    class DummyStore(SessionStoreBase):
        ...
        async def update_state(self, session_id: str, payload: dict, source: str = "api") -> SessionState:
            return self._create_default_session(session_id)
        ...
        async def unsubscribe(self, session_id: str, queue):
            pass
        async def publish_event(self, session_id: str, event_type: str, data: dict, source: str = "api") -> None:
            pass
...
def test_beneficios_http_endpoints_success():
    ...
    # Simular auditoria
    res_aud = client.post("/api/v1/beneficios/simular-auditoria", json={
        "tax_year": 2026,
        "impuesto_neto_ano_anterior": 50000000.0
    })
    assert res_aud.status_code == 200
    assert res_aud.json()["cumple_impuesto_minimo"] is True

    # Simular reduccion
    res_red = client.post("/api/v1/beneficios/simular-reduccion-sancion", json={
