...
def liquidar_persona_juridica(payload: PersonaJuridicaInput) -> PersonaJuridicaOutput:
    ...
    # 1. INGRESOS
    ingresos_brutos = (
        payload.ingresos_brutos_operacionales + payload.ingresos_brutos_no_operacionales
    )
    ingresos_netos = max(
        0.0,
        ingresos_brutos
        - payload.devoluciones_rebajas_descuentos
    ...
    gastos_brutos = (
        payload.gastos_administracion + payload.gastos_ventas + payload.gastos_financieros
    )
    gastos_deducibles = max(
        0.0, gastos_brutos - payload.gastos_no_deducibles + payload.deducciones_especiales
    )
    ...
    total_compensaciones_y_exentas = (
        payload.rentas_exentas
        + payload.compensacion_perdidas_fiscales
        + payload.compensacion_exceso_renta_presuntiva
    )
    ...
    tarifa_aplicada = (
        payload.tarifa_personalizada
        if payload.tarifa_personalizada is not None
        else pj_rules.tarifa_general
    )
    ...
    otros_desc_aplicado = min(
        payload.otros_descuentos_tributarios, max(0.0, impuesto_basico - desc_ica_aplicado)
    )
