...
def liquidar_persona_natural(payload: PersonaNaturalInput) -> PersonaNaturalOutput:
    ...
    total_ingresos = (
        ingresos_trabajo
        + payload.otros_ingresos_brutos
        + payload.rentas_capital
        + payload.rentas_nolaborales
    )
    trace.append(
        AuditTraceItem(
    ...
    total_incrngo = (
        incrngo_trabajo
        + payload.otros_incrngo
        + payload.incrngo_capital
        + payload.incrngo_nolaborales
    )
    trace.append(
        AuditTraceItem(
            step_id="incrngo",
    ...
    if payload.numero_dependientes_adicionales_72uvt > 0:
        ...
        trace.append(
            AuditTraceItem(
                ...
                raw_input_cop=float(
                    payload.numero_dependientes_adicionales_72uvt * uvt_per_dep * uvt
                ),
    ...
    if (
        ...
        and deducciones_dict.compras_factura_electronica_1pct
    ):
    ...
    for bracket in cg_rules.tabla_marginal_art241:
        if bracket.desde_uvt <= renta_liquida_gravable_uvt < bracket.hasta_uvt:
            ...
            impuesto_uvt = (
                renta_liquida_gravable_uvt - bracket.desde_uvt
    ...
    costos_go = min(
        payload.costos_ganancia_ocasional, payload.ganancias_ocasionales_brutas_activos_fijos
    )

    # Exenciones Art. 307
    max_exenta_posible = max(
        0.0,
        (payload.ganancias_ocasionales_brutas_activos_fijos - costos_go)
        + payload.ganancias_ocasionales_brutas_herencias,
    )
    go_exenta_aceptada = min(payload.ganancias_ocasionales_exentas_solicitadas, max_exenta_posible)

    go_gravable_ordinaria = max(
        0.0,
        (payload.ganancias_ocasionales_brutas_activos_fijos - costos_go)
        + payload.ganancias_ocasionales_brutas_herencias
        - go_exenta_aceptada,
    ...
    impuesto_go_cop = (
        round(
            ((go_gravable_ordinaria * tarifa_go_ord) + (go_gravable_loterias * tarifa_go_lot))
            / 1000.0
        )
        * 1000.0
    )
    ...
    renta_liq_nolaboral = max(
        0.0, payload.rentas_nolaborales - payload.incrngo_nolaborales - payload.costos_nolaborales
    )
    ...
    form_210_dict: dict[str, float] = {
        ...
        "c42_renta_liquida_ordinaria_trabajo": max(
            0.0, renta_liq_trabajo - alivios_procedentes_finales
        ),
        ...
        "c91_total_renta_liquida_ordinaria_cedula_general": renta_liq_trabajo
        + renta_liq_capital
        + renta_liq_nolaboral,
        "c92_total_rentas_exentas_deducciones_limitadas": alivios_procedentes_finales,
        "c93_renta_liquida_ordinaria_cedula_general": max(
            0.0,
            (renta_liq_trabajo + renta_liq_capital + renta_liq_nolaboral)
            - alivios_procedentes_finales,
        ),
    ...
    return PersonaNaturalOutput(
        ...
        total_deducciones_solicitadas=sum(d.get("raw", d["allowed"]) for d in deducciones_list)
        if deducciones_list
        else 0.0,
