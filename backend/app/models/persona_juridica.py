...
class PersonaJuridicaInput(BaseModel):
    ...
    ingresos_brutos_no_operacionales: float = Field(
        0.0, description="Rendimientos financieros, dividendos, otros ingresos"
    )
    ...
    ingresos_no_constitutivos_renta: float = Field(
        0.0,
        description="Ingresos no constitutivos de renta ni ganancia ocasional (ej. dividendos no gravados Art. 48/49)",
    )
    ...
    gastos_no_deducibles: float = Field(
        0.0,
        description="Gastos sin soporte electrónico, sanciones, multas o intereses no deducibles",
    )
